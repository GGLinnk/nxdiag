...
#---------------------------------------------------------------------------------
# Pinned, not derived from the directory name, so the output is nxdiag.nro
...
# screentest101), pulled in as the libs/nxdisplaylib git submodule.
TARGET		:=	nxdiag
BUILD		:=	build
...
# nxdiag drives the GPU through deko3d for its graphics-stack probes, and
# serialises its report with json-c (switch-libjson-c portlib).
LIBS	:= -ldeko3d -ljson-c -lnx -lm
